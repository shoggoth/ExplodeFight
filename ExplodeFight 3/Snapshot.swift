...
//  EF3
//
//  Created by Richard Henry on 08/03/2021.
...
//                return agent
//            }
//            else if let playerBot = entity as? PlayerEntity {
...
//            }
        ...
        */
        ...
        let (goodTaskBots, badTaskBots) = scene.entities.reduce(([], [])) {
            ...
            (workingArrays: (goodBots: [GKEntity], badBots: [GKEntity]), thisEntity: GKEntity) -> ([GKEntity], [GKEntity]) in
            
            // Try to cast this entity as a `TaskBot`, and skip this entity if the cast fails.
            guard let thisTaskBot = thisEntity as? GKEntity else { return workingArrays }
        ...
        for entity in scene.entities {
            let entitySnapshot = EntitySnapshot(badBotPercentage: badBotPercentage, proximityFactor: 0.31337, entityDistances: entityDistances[entity]!)
